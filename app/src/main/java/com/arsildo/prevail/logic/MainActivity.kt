...
import com.arsildo.prevail.presentation.screens.ThreadScreen
    ...
    fun NavigationGraph(navController: NavHostController) {
        ...
        NavHost(
            ...
            composable(route = Destinations.Main.route) {
                ...
                ) { threadNumber,threadName ->
                    navController.navigate("${Destinations.ThreadScreen.route}/$threadNumber/$threadName")
                }
            }

            composable(
                route = Destinations.ThreadScreen.route + "/{threadNumber}" +"/{threadName}",
                arguments = listOf(
                    navArgument("threadNumber") {
                        type = NavType.IntType
                        nullable = false
                    },
                    navArgument("threadName") {
                        type = NavType.StringType
                        nullable = false
                    }
                )
            ) {
                val threadViewModel = hiltViewModel<ThreadViewModel>()
                ThreadScreen(
                    navController = navController,
                    viewModel = threadViewModel,
                    threadName = it.arguments?.getString("threadName")!!
                )
            }
...
sealed class Destinations(val route: String) {
    ...
    object ThreadScreen : Destinations(route = "thread_screen")
